import wx from './weapp-promise'
import qcloud from './qcloud-promise'
import wxTunnel from './wx-tunnel'
  ...
  function openConnect () {
    ...
    .then(response => {
      if (+response.statusCode === 200 && response.data && response.data.url) {
        return openSocket(me.socketUrl = response.data.url)
      } else {
    ...
    .catch(err => dispatchConnectServiceError(err))
